...
for account in neon_accounts:
    ...
    if dID:
        ...
        if neonAccountsByDID.get(dID):
            logging.warning(
                f"""Neon Account Problem: duplicate use of discourseID {dID}"""
            )
        else:
...
if os.path.exists(discourseFilename) and os.access(discourseFilename, os.R_OK):
    ...
    while True:
        ...
        if len(memberResponse) < 100:
            ...
            logging.info(
                f"{len(fullDlist)} active users retrieved from Discourse... Querying for more data."
            )
            page += 1
    ...
    for i, response in enumerate(fullDlist):
        ...
        if neonAccountsByDID.get(dID):
            ...
            if " " in dID or "|" in dID:
                logging.warning(
                    f'Discourse username "{dID}" found is not in a valid format. Check response at index {i}.'
                )
            else:
                # print("All good")
                continue
    ...
    logging.info(
        f"{len(fullDlist)} active users retrieved from Discourse... saving to file as {discourseFilename}"
    )
...
for acct in neonAccountList:
    ...
    if acct.get("DiscourseID") is not None:
        ...
        if any(dUser.get("username") == dID for dUser in fullDlist):
            ...
            logging.warning(
                f"""Neon Account Problem: {fullname}'s dID "{dID}" doesn't actually exist in Discourse (Neon ID {neonID})"""
            )
    else:
    ...
    for Daccount in fullDlist:
        if (
            Daccount.get("email")
            and Daccount.get("email").casefold() == email.casefold()
        ):
            ...
            Daccount.get("name")
            and Daccount.get("name").casefold() == fullname.casefold()
        ):
            dID = Daccount.get("username")
    ...
    if dID != "":
        fixedDiscourseIDs += 1
        logging.info(
            f"   Updating DiscourseID to {dID} for Neon account #{neonID} - {fullname} {email}"
        )
        acct["DiscourseID"] = dID
        neonUtil.updateDID(acct)
    else:
        logging.debug(
            f"   No DiscourseID found for Neon account #{neonID} - {fullname} {email}"
        )
        noDIDlist.append(acct)

logging.info(
    f"""{len(neonAccountList)} Neon accounts checked: {matchedDiscourseIDs} had valid DiDs, {fixedDiscourseIDs} new matches, {len(noDIDlist)} still missing DiDs"""
)
...
with open(neonDImissingFilename, "w") as outfile:
    logging.info(
        f"Missing-DiDs Report saved to file as {neonDImissingFilename} for further review."
    )
